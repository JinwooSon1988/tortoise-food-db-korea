...
def series(name):
 ...
 return out
ass=series("assessments")
priority={"Mediterranean_Testudo":0,"Tortoise_general":1,"Herbivorous_reptile_general":2}
by={}
for a in ass:
...
def cards(items):
 out=[]
 for p in items:
  a=by.get(p["id"]); verdict=VL.get((a or {}).get("verdict"),"판정 보류")
  why=(a or {}).get("why") or "공개 판정을 위한 근거 검토가 완료되지 않았다."
  scope=(a or {}).get("applicability_note") or (a or {}).get("species_group") or "적용 범위 검토 중"
...
urls=[
 page("market-foods","마트에서 구할 수 있는 육지거북 먹이","마트·시장·온라인에서 찾기 쉬운 육지거북 먹이를 DB 판정과 근거 범위로 비교한다.",market,"한국에서 실제로 구하기 쉬운 먹이를 모았다. 편의성보다 종 동정과 근거 수준을 우선해 판단한다."),
 page("wild-plants","육지거북 야생초·채집 식물","민들레·질경이 등 육지거북 야생초와 채집 식물의 판정, 적용 범위와 근거를 확인한다.",wild,"야생초는 자연식이라는 이유만으로 자동 안전하지 않다. 식물 동정, 오염 가능성, 대상 종에 대한 근거를 함께 확인한다."),
 page("caution-foods","육지거북 급여 주의 식물","제한적 보조근거·일반 파충류 근거·동정 주의가 필요한 육지거북 먹이를 구분해 확인한다.",caution,"‘먹을 수 있음’과 ‘주식으로 적합함’을 분리한다. 직접 근거가 약하거나 적용 범위가 좁은 항목을 우선 확인한다.")
]
