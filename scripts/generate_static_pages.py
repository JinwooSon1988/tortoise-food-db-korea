...
<style>:root{{--bg:#f6f8f5;--card:#fff;--line:#dce5dd;--text:#17231b;--muted:#647067;--green:#e7f5eb;--yellow:#fff7df;--hold:#f1f3f2}}*{{box-sizing:border-box}}body{{font-family:system-ui,-apple-system,"Noto Sans KR",sans-serif;max-width:1180px;margin:auto;padding:22px;line-height:1.62;color:var(--text);background:var(--bg)}}a{{color:inherit}}.card{{background:var(--card);border:1px solid var(--line);border-radius:16px;padding:16px;margin:12px 0}}.verdict{{font-weight:900;font-size:20px}}.green{{background:var(--green)}}.yellow{{background:var(--yellow)}}.hold{{background:var(--hold)}}.grid{{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}}.grid>div{{background:#fff;border:1px solid var(--line);border-radius:12px;padding:12px}}.warn{{border-left:5px solid #866f25}}.small{{font-size:13px;color:var(--muted)}}.relatedgrid{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}}.related{{display:block;border:1px solid var(--line);border-radius:12px;padding:11px;text-decoration:none;font-weight:800;background:#fff}}.share{{display:flex;gap:8px;flex-wrap:wrap}}.share button,.share a{{border:0;border-radius:12px;padding:11px 14px;font-weight:800;background:#e7f5eb;text-decoration:none;cursor:pointer}}h1{{margin-bottom:6px}}h2{{font-size:19px;margin:0 0 8px}}ul{{padding-left:20px}}@media(max-width:800px){{body{{padding:14px}}.grid{{grid-template-columns:1fr 1fr}}.relatedgrid{{grid-template-columns:1fr 1fr}}}}@media(max-width:520px){{.grid,.relatedgrid{{grid-template-columns:1fr}}}}</style></head><body>
<p><a href="../../index.html">← 거북밥 DB 검색으로</a></p><h1>육지거북, {esc(ko)} 먹어도 될까?</h1><div class="small"><i>{esc(sci)}</i></div><section class="card {tone}"><h2>1. 결론 — {esc(ko)} 급여 판정</h2><div class="verdict">{esc(label)}</div><p>{esc(summary)}</p><p class="small"><b>판정 해석:</b> 이 결론은 아래에 표시된 적용 범위와 근거 수준 안에서만 유효하다. 혼합급여 가능 판정은 단독 주식·무제한 급여·고정 급여비율을 의미하지 않는다.</p></section><section class="card"><h2>2. 이 판정은 어디까지 적용되는가</h2><div class="grid">{role_html}{evidence_html}<div><b>식물 이름</b><br>{esc(', '.join(aliases))}</div><div><b>학명 표기</b><br><i>{esc(sci)}</i></div></div></section><section class="card warn"><h2>3. 식물동정 — 이름이 같아도 같은 식물은 아니다</h2><p>{esc(identity_text)}</p>{'<p><b>아욱 주의:</b> 한국 유통명 아욱을 Malva parviflora로 자동 매핑하지 않는다.</p>' if pid=='mallow' else ''}</section><section class="card"><h2>4. 근거의 한계와 해석 주의</h2><ul>{limits_html}</ul><p class="small">야생에서 먹었다는 기록 ≠ 무제한 급여 권장. 사람용 영양자료 ≠ 육지거북 독성 한계치. 근거 부족 ≠ 안전.</p></section><section class="card"><h2>5. 추가로 확인할 식물</h2><div class="relatedgrid">{related_html}</div><p class="small"><b>탐색 링크:</b> 같은 과·카테고리의 다른 항목으로 이동하기 위한 기능이다. 식물학적 유사성이 동일한 급여 안전성·영양가·권장도를 뜻하지 않는다.</p></section><section class="card"><h2>이 판정 공유하기</h2><div class="share"><button type="button" onclick="navigator.clipboard.writeText(location.href).then(()=>this.textContent='링크 복사 완료')">링크 복사</button><a href="../../all-plants/">다른 먹이 찾기 →</a></div></section><nav class="small" aria-label="breadcrumb"><a href="../../index.html">거북밥 DB</a> › {esc(ko)}</nav></body></html>'''
    (d/"index.html").write_text(doc,encoding="utf-8")
print("generated",len(plants),"non-candidate/public plant detail pages from",len(all_plants),"master records and",len(assessments),"assessments")