...
for p in plants:
    ...
    title=f"{ko} 육지거북 먹이 판정 | 거북밥 DB Korea"; desc=f"{ko}의 육지거북 급여 적합성, 식물동정, 근거와 검증 상태를 확인한다."; canonical=f"{SITE_URL}/plant/{pid}/"
...
<p><a href="../../index.html">← 거북밥 DB 검색으로</a></p><h1>{esc(ko)}</h1><div class="small"><i>{esc(sci)}</i></div><section class="card {tone}"><div class="verdict">{esc(label)}</div><p>{esc(summary)}</p></section><section class="card"><h2>판정 범위</h2><div class="grid">{role_html}{evidence_html}<div><b>식물 이름</b><br>{esc(', '.join(aliases))}</div><div><b>학명 표기</b><br><i>{esc(sci)}</i></div></div></section><section class="card warn"><h2>식물동정 주의</h2><p>{esc(identity_text)}</p>{'<p><b>아욱 주의:</b> 한국 유통명 아욱을 Malva parviflora로 자동 매핑하지 않는다.</p>' if pid=='mallow' else ''}</section><section class="card"><h2>현재 근거의 한계</h2><ul>{limits_html}</ul><p class="small">야생에서 먹었다는 기록 ≠ 무제한 급여 권장. 사람용 영양자료 ≠ 육지거북 독성 한계치. 근거 부족 ≠ 안전.</p></section><nav class="small" aria-label="breadcrumb"><a href="../../index.html">거북밥 DB</a> › {esc(ko)}</nav></body></html>'''
