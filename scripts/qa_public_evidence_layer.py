...
for i,r in enumerate(data.get("records",[])):
    ...
    if r.get("animal_taxon")=="not_applicable" and r.get("applicability")!="composition_only": errors.append(f"{label}: animal_taxon=not_applicable is only valid for composition_only evidence")
    if r.get("applicability")=="composition_only" and r.get("animal_taxon")==r.get("plant_taxon"): errors.append(f"{label}: composition_only evidence must not duplicate plant_taxon into animal_taxon")
