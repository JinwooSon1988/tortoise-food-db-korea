...
Prepared assets: `data/evidence_korea_addendum_10.json`, `data/dill_candidate_packet.json`, and `data/dill_candidate_assessment.json`. Candidate staging is complete and both candidate and assessment are marked ready for atomic promotion, while the assessment remains `staged_not_runtime` and `user_facing: false` until that release.
