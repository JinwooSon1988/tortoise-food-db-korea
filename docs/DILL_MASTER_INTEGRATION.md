...
Prepared assets: `data/evidence_korea_addendum_10.json`, `data/dill_candidate_packet.json`, and `data/dill_candidate_assessment.json`. The assessment is intentionally marked `staged_not_runtime` and `user_facing: false` until the master/UI release is atomic. The packet is now marked ready for that atomic promotion.
