...
Normalize the public profile context in `profile-context.js` by adding derived `species_name` and `weight_g` fields while preserving the original stored object. This avoids destructive localStorage migration and maintains compatibility with v4.6 data.
